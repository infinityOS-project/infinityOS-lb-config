...
export class BaseMenuLayout extends St.BoxLayout {
    ...
    constructor(menuButton, params) {
        ...
        GLib.idle_add(GLib.PRIORITY_DEFAULT, () => {
            try {
                if (typeof Shell !== 'undefined' && Shell.BlurEffect) {
                    this._blurEffect = new Shell.BlurEffect({
                        mode: Shell.BlurMode.BACKGROUND
                    });
                    
                    // Try direct property assignment (works on most GNOME versions)
                    this._blurEffect.sigma = 30;
                    this._blurEffect.brightness = 0.6;
                    
                    // If 'sigma' is still picky, some versions use 'blur-radius' 
                    // but as a property, not a function:
                    // this._blurEffect['blur-radius'] = 30;

                    this.add_effect(this._blurEffect);
                }
            } catch (e) {
                console.log(`ArcMenu Blur Error: ${e}`);
            }
