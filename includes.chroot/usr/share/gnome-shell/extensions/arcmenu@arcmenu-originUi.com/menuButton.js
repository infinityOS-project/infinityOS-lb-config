...
const transformScaleX = 50;
const transformScaleY = 40;
const iconFadeInScale = 5;
const iconFadeInSpeed = 310;
const iconFadeInDelayRatio = 180;
const iconFadeInCubicBezier = [0.25, 1.1, 0.12, 1];
const iconFadeInFrameInterval = 1000 / 60;
...
function cubicDerivative(a, b, t) {
  const inv = 1 - t;
...
function cubicBezierProgress(x1, y1, x2, y2, progress) {
  ...
  let t = progress;
  for (let i = 0; i < 8; i++) {
    const x = cubic(x1, x2, t) - progress;
    const dx = cubicDerivative(x1, x2, t);
...
export const MenuButton = GObject.registerClass(
  class ArcMenuMenuButton extends PanelMenu.Button {
    _init(panelInfo, monitorIndex) {
      ...
      this.arcMenuContextMenu.connectObject(
        "open-state-changed",
        this._onOpenStateChanged.bind(this),
        this,
      );
    ...
    initiate() {
      ...
      if (
        this._dtp?.state === Utils.ExtensionState.ACTIVE &&
        global.dashToPanel
      )
    ...
    syncWithDashToPanel() {
      ...
      this._dtpSettings = dtp.getSettings(
    ...
    setMenuPositionAlignment() {
      ...
      const arrowAlignment =
        1 - ArcMenuManager.settings.get_int("menu-position-alignment") / 100;
      const panelPosition =
        ArcMenuManager.settings.get_enum("position-in-panel");
    ...
    updateArrowSide(side, setAlignment = true) {
      let arrowAlignment;
      if (side === St.Side.RIGHT || side === St.Side.LEFT) arrowAlignment = 1.0;
      else arrowAlignment = 0.5;
      ...
      if (setAlignment) this.setMenuPositionAlignment();
    }
    ...
    _getDashToPanelGeom() {
      ...
      const menuLocation = ArcMenuManager.settings.get_enum(
        "force-menu-location",
      );
      ...
      const leftLocations = [
        MenuLocation.BOTTOM_LEFT,
        MenuLocation.TOP_LEFT,
        MenuLocation.LEFT_CENTERED,
      ];
      const rightLocations = [
        MenuLocation.BOTTOM_RIGHT,
        MenuLocation.TOP_RIGHT,
        MenuLocation.RIGHT_CENTERED,
      ];
      const xCenterLocations = [
        MenuLocation.BOTTOM_CENTERED,
        MenuLocation.TOP_CENTERED,
      ];
      const yCenterLocations = [
        MenuLocation.LEFT_CENTERED,
        MenuLocation.RIGHT_CENTERED,
      ];

      const needsTopAdjustment =
        topLocations.includes(menuLocation) ||
        yCenterLocations.includes(menuLocation);
      const needsBottomAdjustment =
        bottomLocations.includes(menuLocation) ||
        yCenterLocations.includes(menuLocation);
      const needsLeftAdjustment =
        leftLocations.includes(menuLocation) ||
        xCenterLocations.includes(menuLocation);
      const needsRightAdjustment =
        rightLocations.includes(menuLocation) ||
        xCenterLocations.includes(menuLocation);
    ...
    forceMenuLocation() {
      const layout = ArcMenuManager.settings.get_enum("menu-layout");
      if (
        layout === Constants.MenuLayout.RUNNER ||
      ...
      const newMenuLocation = ArcMenuManager.settings.get_enum(
        "force-menu-location",
      );
      if (this._menuLocation !== newMenuLocation) {
        ...
        this.arcMenu.sourceActor = this.arcMenu.focusActor =
          Main.layoutManager.dummyCursor;
        this.arcMenu._boxPointer.setPosition(
          Main.layoutManager.dummyCursor,
          0.5,
        );
        this.arcMenu._boxPointer.setSourceAlignment(0.5);
      ...
      if (newMenuLocation === Constants.MenuLocation.OFF) return;
      ...
      const yCentered = Math.round(
        monitor.y + monitor.height / 2 - menuHeight / 2,
      );
      let x, y;
    ...
    _getClickActionForButton(button) {
      if (button === Clutter.BUTTON_PRIMARY)
        return ArcMenuManager.settings.get_enum(
    ...
    closeContextMenu() {
      if (this.arcMenuContextMenu.isOpen) this.arcMenuContextMenu.toggle();
    }
    ...
    showTooltip(sourceActor, location, titleLabel, description, displayType) {
      this.clearTooltipShowingId();
      this._tooltipShowingID = GLib.timeout_add(
        GLib.PRIORITY_DEFAULT,
        750,
        () => {
          this._tooltip.setTooltipData(
            sourceActor,
            location,
            titleLabel,
            description,
    ...
    setDefaultMenuView() {
      ...
      if (!this._menuLayout.reloadQueued) this._menuLayout.setDefaultMenuView();
    }
    ...
    _onOpenStateChanged(menu, open) {
      ...
      if (open) {
        ...
        const closeOverview = ArcMenuManager.settings.get_boolean(
          "hide-overview-on-arcmenu-open",
        );
        if (isArcMenu && closeOverview) Main.overview.hide();
        ...
        if (this.arcMenu.isOpen || this.arcMenuContextMenu.isOpen) return;
    ...
    _maybeHidePanel(callback) {
      ...
      if (isMouseOnPanel) this._startTrackingMouse(callback);
      else callback();
    }

    _maybeShowPanel() {
      if (
        this._panelParent.intellihide &&
        this._panelParent.intellihide.enabled
      ) {
    ...
    _panelChildHasGrab() {
      if (this._panelBox.appIconsTaskbar?.menuManager?.activeMenu) return true;

      const grabActor = global.stage.get_grab_actor();
      if (!grabActor) return false;
      ...
      return (
        ...
        quickSettingsMenu?.contains(sourceActor)
      );
    }
...
export const ArcMenu = class ArcMenuArcMenu extends PopupMenu.PopupMenu {
  constructor(sourceActor, arrowAlignment, arrowSide, parent) {
    ...
    this.actor.add_style_class_name("arcmenu-grid-animation-prehide");
    this.actor.hide();
    ...
    this._blurEffect.sigma = 30;
    this._blurEffect.mode = Shell.BlurMode.BACKGROUND;
    this._blurEffect.brightness = 0.6;
    this._boxPointer.add_effect_with_name("blur", this._blurEffect);

    this._gridAnimationIds = new Set();
    this._gridAnimationItems = new Set();
  }

  _onCapturedEvent(actor, event) {
    if (Main.keyboard.maybeHandleEvent(event)) return Clutter.EVENT_STOP;
  ...
  open(animate) {
    ...
    super.open(animate);
    this._animateGridItemsFromParentOrigin();
  }

  close(animate) {
    this._menuButton?.onArcMenuClose();
    this._clearGridAnimation();
    this.actor.add_style_class_name("arcmenu-grid-animation-prehide");
  ...
  destroy() {
    ...
    this._boxPointer.remove_effect_by_name("blur");
    super.destroy();
  ...
  _clearGridAnimation() {
    if (!this._gridAnimationIds) return;

    for (const id of this._gridAnimationIds) GLib.Source.remove(id);

    this._gridAnimationIds.clear();
    this._restoreGridAnimationItems();
  }

  _removeGridAnimationId(id) {
    this._gridAnimationIds?.delete(id);
  }

  _trackGridAnimationItem(item) {
    this._gridAnimationItems?.add(item);
  }

  _untrackGridAnimationItem(item) {
    this._gridAnimationItems?.delete(item);
  }

  _restoreGridAnimationItem(item) {
    if (!item || item.isDestroyed || item._destroyed) return;

    try {
      if (!item.get_parent?.()) return;
  ...
  _restoreGridAnimationItems() {
    if (!this._gridAnimationItems) return;

    for (const item of this._gridAnimationItems)
      this._restoreGridAnimationItem(item);

    this._gridAnimationItems.clear();
  }

  _isItemInsideOriginParent(item, originParent) {
    if (!item.has_allocation() || !originParent.has_allocation()) return false;

    const itemParent = item.get_parent?.();
    if (!itemParent?.has_allocation()) return false;

    const [itemParentX, itemParentY] = itemParent.get_transformed_position();
    const [originX, originY] = originParent.get_transformed_position();
    const box = item.allocation;
    const itemX1 = itemParentX + box.x1;
    const itemY1 = itemParentY + box.y1;
    const itemX2 = itemParentX + box.x2;
    const itemY2 = itemParentY + box.y2;

    return (
      itemX2 > originX &&
      itemX1 < originX + originParent.width &&
      itemY2 > originY &&
      itemY1 < originY + originParent.height
    );
  }

  _isGridAnimationItem(actor) {
    if (!actor.visible || !actor.has_allocation()) return false;

    if (actor.has_style_class_name?.("ArcMenuIconGrid")) return true;

    const parent = actor.get_parent?.();
    return (
      actor instanceof MW.BaseMenuItem &&
      actor._iconBin &&
      parent?.constructor?.name?.includes("IconGrid")
    );
  }

  _collectGridAnimationItems(actor, items = []) {
    if (this._isGridAnimationItem(actor)) items.push(actor);

    const children = actor.get_children?.() ?? [];
    for (const child of children) this._collectGridAnimationItems(child, items);

    return items;
  }

  _animateGridItemsFromParentOrigin() {
    this._clearGridAnimation();

    const laterId = GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
      this._removeGridAnimationId(laterId);

      if (!this.isOpen) return GLib.SOURCE_REMOVE;

      const items = this._collectGridAnimationItems(this.actor);
      if (items.length === 0) {
        this.actor.remove_style_class_name("arcmenu-grid-animation-prehide");
        return GLib.SOURCE_REMOVE;
      }
      ...
      for (const item of items) {
        ...
        const originParent = parent?.get_parent?.() ?? parent;

        if (
          !parent ||
          !parent.has_allocation() ||
          !originParent ||
          !originParent.has_allocation()
        )
          continue;

        if (!this._isItemInsideOriginParent(item, originParent)) continue;

        if (!groups.has(originParent)) groups.set(originParent, []);

        groups.get(originParent).push(item);
      }

      for (const [originParent, groupItems] of groups)
        this._animateGridItemGroup(originParent, groupItems);
    ...
    this._gridAnimationIds.add(laterId);
  }

  _getOriginInItemParent(originParent, itemParent) {
    const [originStageX, originStageY] =
      originParent.get_transformed_position();
    const [itemParentStageX, itemParentStageY] =
      itemParent.get_transformed_position();

    return [
      originStageX +
        (originParent.width * transformScaleX) / 100 -
        itemParentStageX,
      originStageY +
        (originParent.height * transformScaleY) / 100 -
        itemParentStageY,
    ];
  }

  _animateGridItemGroup(originParent, items) {
    let maxDistance = 1;
    ...
    for (const item of items) {
      ...
      const [originX, originY] = this._getOriginInItemParent(
        originParent,
        itemParent,
      );
      ...
      item.scale_y = 1;
      item.opacity = 0;
      this._trackGridAnimationItem(item);

      if (typeof item.set_pivot_point === "function")
        item.set_pivot_point(0.5, 0.5);

      const box = item.allocation;
      const itemX = (box.x1 + box.x2) / 2;
      const itemY = (box.y1 + box.y2) / 2;
      const distance = Math.hypot(itemX - originX, itemY - originY);

      item._arcMenuGridAnimationDistance = distance;
    ...
    for (const item of items) {
      ...
      const itemY = (box.y1 + box.y2) / 2;
      const delay =
        (item._arcMenuGridAnimationDistance / maxDistance) *
        iconFadeInDelayRatio;
      const fromX = (itemX - originX) * (iconFadeInScale + delay / 250 - 1);
      const fromY = (itemY - originY) * (iconFadeInScale + delay / 250 - 1);

      item.translation_x = fromX;
      item.translation_y = fromY;
      item.scale_x = iconFadeInScale;
      item.scale_y = iconFadeInScale;
      item.opacity = 0;
      this._trackGridAnimationItem(item);

      this._easeGridItem(item, fromX, fromY, iconFadeInScale, delay);
    }
  }

  _easeGridItem(item, fromX, fromY, fromScale, delay) {
    const start = () => {
      ...
      frameId = GLib.timeout_add(
        GLib.PRIORITY_DEFAULT,
        iconFadeInFrameInterval,
        () => {
          try {
            const elapsed = (GLib.get_monotonic_time() - startTime) / 1000;
            const linear = elapsed / (iconFadeInSpeed + 3.8 * delay);

            const eased = cubicBezierProgress(
              ...[0.25, 1.23 - delay / 1750, 0.1, 1],
              linear,
            );
            const inv = 1 - eased;

            item.translation_x = fromX * inv;
            item.translation_y = fromY * inv;
            item.scale_x = 1 + (fromScale - 1) * inv;
            item.scale_y = 1 + (fromScale - 1) * inv;
            item.opacity = Math.clamp(Math.round(255 * eased), 0, 255);
          ...
          this._removeGridAnimationId(frameId);
          this._untrackGridAnimationItem(item);
          return GLib.SOURCE_REMOVE;
        },
      );

      this._gridAnimationIds.add(frameId);
      return GLib.SOURCE_REMOVE;
    };
    ...
    let delayId = 0;
    delayId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, delay / 2, () => {
      this._removeGridAnimationId(delayId);
      start();
      return GLib.SOURCE_REMOVE;
    });
    this._gridAnimationIds.add(delayId);
  }
};

var ArcMenuContextMenu = class ArcMenuArcMenuContextMenu
  extends PopupMenu.PopupMenu
{
  ...
  populateMenuItems() {
    ...
    const contextMenuShortcuts = ArcMenuManager.settings
      .get_value("context-menu-items")
      .deep_unpack();
    ...
    for (let i = 0; i < contextMenuShortcuts.length; i++) {
      ...
      if (id.endsWith(".desktop")) {
        ...
        if (settingsPage === "About")
          ...
          this.addArcMenuSettingsItem(
            name,
            Constants.SettingsPage.CUSTOMIZE_MENU,
          );
          ...
          this.addArcMenuSettingsItem(
            name,
            Constants.SettingsPage.BUTTON_APPEARANCE,
          );
  ...
  disconnectPowerOptions() {
    if (this.canSuspendId) this._systemActions.disconnect(this.canSuspendId);
  ...
  addPowerOptionsMenuItem() {
    ...
    powerOptionsItem.menu.addAction(_("Restart..."), () =>
      this._systemActions.activateRestart(),
    );
    powerOptionsItem.menu.addAction(_("Power Off..."), () =>
      this._systemActions.activatePowerOff(),
    );

    powerOptionsItem.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());

    powerOptionsItem.menu.addAction(_("Lock"), () =>
      this._systemActions.activateLockScreen(),
    );
    powerOptionsItem.menu.addAction(_("Log Out..."), () =>
      this._systemActions.activateLogout(),
    );
    const switchUserItem = powerOptionsItem.menu.addAction(
      _("Switch User"),
      () => this._systemActions.activateSwitchUser(),
    );
    switchUserItem.visible = this._systemActions.canSwitchUser;

    this.canSuspendId = this._systemActions.connect(
      "notify::can-suspend",
      () => (suspendItem.visible = this._systemActions.canSuspend),
    );
    this.canSwitchUserId = this._systemActions.connect(
  ...
  addSettingsAction(title, desktopFile) {
    ...
    if (!title) title = app.get_name();

    super.addSettingsAction(title, desktopFile);
  }

  addExtensionSettings() {
    const dashToPanel = Main.extensionManager.lookup(
      Constants.DASH_TO_PANEL_UUID,
    );
    const azTaskbar = Main.extensionManager.lookup(Constants.AZTASKBAR_UUID);

    if (
      dashToPanel?.state === Utils.ExtensionState.ACTIVE &&
      global.dashToPanel
    ) {
      const item = new PopupMenu.PopupMenuItem(_("Dash to Panel Settings"));
      item.connect("activate", () =>
        Utils.openPrefs(Constants.DASH_TO_PANEL_UUID),
      );
      this.addMenuItem(item);
    } else if (
      azTaskbar?.state === Utils.ExtensionState.ACTIVE &&
      global.azTaskbar
    ) {
