...
export const AppContextMenu = class ArcMenuAppContextMenu extends AppMenu {
  constructor(sourceActor, menuLayout) {
    ...
    this._arcMenuPinnedItem = this._createMenuItem(
      _("Pin to ArcMenu"),
      8,
      () => {
        this.close();
        ...
        if (this._pinnedAppData) {
          ...
          else sourceSettings = ArcMenuManager.settings;

          const pinnedAppsList = sourceSettings
            .get_value("pinned-apps")
            .deepUnpack();
          for (let i = 0; i < pinnedAppsList.length; i++) {
            if (pinnedAppsList[i].id === this._app.get_id()) {
              pinnedAppsList.splice(i, 1);
              sourceSettings.set_value(
                "pinned-apps",
                new GLib.Variant("aa{ss}", pinnedAppsList),
              );
              break;
            }
          }
        } else {
          const pinnedAppsList = ArcMenuManager.settings
            .get_value("pinned-apps")
            .deepUnpack();
          ...
          ArcMenuManager.settings.set_value(
            "pinned-apps",
            new GLib.Variant("aa{ss}", pinnedAppsList),
          );
        }
    ...
    this._createDesktopShortcutItem = this._createMenuItem(
  ...
  setApp(app) {
    ...
    actions.forEach((action) => {
      ...
      this._actionSection.addAction(label, (event) => {
        if (action === "new-window") this._animateLaunch();
  ...
  getDesktopShortcut() {
    if (!this._app) return [false, null, null];
    ...
    const dst = Gio.File.new_for_path(
      GLib.build_filenamev([desktop, src.get_basename()]),
    );
  ...
  _updateDesktopShortcutItem() {
    ...
    if (!this._app || !this._createDesktopShortcutItem) return;

    const [exists] = this.getDesktopShortcut();
    this._createDesktopShortcutItem.label.text = exists
    ...
    this._arcMenuPinnedItem = this._createMenuItem(
      ...
      () => {
        ...
        if (folder) {
          ...
          for (const key of keys) folder.reset(key);
  ...
  _updateArcMenuPinnedItem() {
    ...
    this._arcMenuPinnedItem.label.text = this._pinnedAppData
      ? _("Unpin from ArcMenu")
      : _("Pin to ArcMenu");
  }
  ...
  _updateWindowsSection() {
    ...
    if (windows.length < minWindows) return;
  ...
  setFolderPath(path) {
    ...
    this._openFolderLocationItem = this._createMenuItem(
      _("Open Folder Location"),
      0,
      () => {
        const file = Gio.File.new_for_path(path);
        const context = global.create_app_launch_context(
          Clutter.get_current_event().get_time(),
          -1,
        );
