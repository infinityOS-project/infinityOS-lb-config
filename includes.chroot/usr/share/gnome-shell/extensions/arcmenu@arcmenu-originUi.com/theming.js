...
export class CustomStylesheet {
  constructor(settings) {
    ...
    connectSettings(
      [
        ...
        "menu-item-hover-fg-color",
        "menu-item-active-bg-color",
        "menu-button-border-color",
        "menu-item-active-fg-color",
        "menu-button-fg-color",
        "menu-button-bg-color",
        "menu-arrow-rise",
        "menu-button-hover-bg-color",
        "menu-button-hover-fg-color",
        "menu-button-active-bg-color",
        "menu-button-active-fg-color",
  ...
  async _update() {
    if (!this._stylesheet) {
      console.log(
        "ArcMenu - Warning: Custom stylesheet not found! Unable to set contents of custom stylesheet.",
      );
      return;
    }
    ...
    const customThemeEnabled = this._settings.get_boolean(
      "override-menu-theme",
    );
    const [menuRise, menuRiseValue] = this._settings
      .get_value("menu-arrow-rise")
      .deep_unpack();
    const [buttonFG, buttonFGColor] = this._settings
      .get_value("menu-button-fg-color")
      .deep_unpack();
    const [buttonBG, buttonBGColor] = this._settings
      .get_value("menu-button-bg-color")
      .deep_unpack();
    const [buttonHoverBG, buttonHoverBGColor] = this._settings
      .get_value("menu-button-hover-bg-color")
      .deep_unpack();
    const [buttonHoverFG, buttonHoverFGColor] = this._settings
      .get_value("menu-button-hover-fg-color")
      .deep_unpack();
    const [buttonActiveBG, buttonActiveBGColor] = this._settings
      .get_value("menu-button-active-bg-color")
      .deep_unpack();
    const [buttonActiveFG, buttonActiveFGColor] = this._settings
      .get_value("menu-button-active-fg-color")
      .deep_unpack();
    const [buttonRadius, buttonRadiusValue] = this._settings
      .get_value("menu-button-border-radius")
      .deep_unpack();
    const [buttonWidth, buttonWidthValue] = this._settings
      .get_value("menu-button-border-width")
      .deep_unpack();
    const [buttonBorder, buttonBorderColor] = this._settings
      .get_value("menu-button-border-color")
      .deep_unpack();
    const [searchBorder, searchBorderValue] = this._settings
      .get_value("search-entry-border-radius")
      .deep_unpack();

    let css = "";

    if (buttonFG) css += `.arcmenu-menu-button{ color: ${buttonFGColor}; }`;
    if (buttonBG)
    ...
    if (buttonWidth)
      css += `.arcmenu-panel-menu{ border-top-width: ${buttonWidthValue}px; border-bottom-width: ${buttonWidthValue}px;
     }`;
    if (buttonBorder)
      css += `.arcmenu-panel-menu{ border-color: ${buttonBorderColor}; }`;
    if (menuRise) css += `.arcmenu-menu{ -arrow-rise: ${menuRiseValue}px; }`;
    if (searchBorder)
    ...
    try {
      ...
      const [success, etag_] =
        await this._stylesheet.replace_contents_bytes_async(
          bytes,
          null,
          false,
      ...
      if (!success) {
        console.log(
          "ArcMenu - Failed to replace contents of custom stylesheet.",
        );
        return;
      }
      ...
      console.log(
        `ArcMenu - Error replacing contents of custom stylesheet: ${e}`,
      );
    }
  }

  _buildCustomCSS() {
    const menuBGColor = "rgba(0, 0, 0, 0.8)";
    const menuFGColor = "rgb(255, 255, 255)";
    const menuBorderColor = "rgba(255, 255, 255, 0.1)";
    const menuBorderWidth = 1;
    const menuBorderRadius = 50;
    const menuFontSize = 11;
    ...
    .arcmenu-menu .popup-menu-item:hover,
    .arcmenu-menu StButton:hover {
    ...
    #ArcMenuSearchEntry:focus {
        box-shadow: inset 0 0 0 2px rgba(255, 255, 255, 0.5);
    }
    .arcmenu-menu .popup-menu-content {
        background-color: ${menuBGColor};
        border-color: ${menuBorderColor};
        border-top-width: ${menuBorderWidth}px;
        border-bottom-width: ${menuBorderWidth}px;
    ...
    .arcmenu-menu StScrollBar StButton#vhandle, .arcmenu-menu StScrollBar StButton#hhandle {
        background-color: ${this._adjustLuminance(menuBGColor, 0.5)};
    }
    ...
    .arcmenu-menu StEntry{
        font-size: ${menuFontSize}pt;
        color: ${menuFGColor};
        background-color: ${this._adjustLuminance(menuBGColor, 0.1, 0.5)};
    }
    .arcmenu-menu StEntry:hover{
        background-color: ${this._adjustLuminance(menuBGColor, 0.15, 0.5)};
    }
    ...
    #ArcMenu_Tooltip{
        ...
        border-top: 1px solid ${this._adjustLuminance(menuBorderColor, 0.25)};
        border-bottom: 1px solid ${this._adjustLuminance(menuBorderColor, 0.25)};
    }
    .arcmenu-menu .user-icon{
        border-color: ${this._adjustLuminance(menuFGColor, 0, 0.7)};
    }`;
  }
  ...
  _adjustLuminance(colorString, luminanceAdjustment, overrideAlpha = null) {
    ...
    const modifiedLuminance =
      (luminance >= 0.85 && luminanceAdjustment > 0) ||
      (luminance <= 0.15 && luminanceAdjustment < 0)
        ? Math.max(Math.min(luminance - luminanceAdjustment, 1), 0)
        : Math.max(Math.min(luminance + luminanceAdjustment, 1), 0);

    let alpha = (color.alpha / 255).toPrecision(3);
    if (overrideAlpha) alpha = overrideAlpha;
