...
const delayRatio = 25; // delay += delayRatio for each row
  ...
  _setupQuickSettingsHook() {
    ...
    for (const menu of menus) {
      ...
      const signalId = menu.connect("open-state-changed", (_menu, isOpen) => {
        if (isOpen) this._animateQuickSettingsOpen(_menu);
        else this._stopAllAnimations(true);
      });
