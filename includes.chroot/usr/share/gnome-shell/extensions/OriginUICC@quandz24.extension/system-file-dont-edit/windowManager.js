// đây là file system ko phải trong extension
...
export const WindowDimmer = GObject.registerClass(
  class WindowDimmer extends Clutter.BrightnessContrastEffect {
    ...
    _syncEnabled(dimmed) {
      let animating =
        this.actor.get_transition(`@effects.${this.name}.brightness`) !== null;

      this.enabled =
        Meta.prefs_get_attach_modal_dialogs() && (animating || dimmed);
    }
...
class WorkspaceTracker {
  ...
  _checkWorkspaces() {
    ...
    if (this._pauseWorkspaceCheck) return GLib.SOURCE_CONTINUE;
    ...
    for (i = lastIndex; i >= 0; i--) {
      if (workspaceManager.n_workspaces === MIN_NUM_WORKSPACES) break;
  ...
  keepWorkspaceAlive(workspace, duration) {
    if (workspace._keepAliveId) GLib.source_remove(workspace._keepAliveId);

    workspace._keepAliveId = GLib.timeout_add(
      GLib.PRIORITY_DEFAULT,
      duration,
      () => {
  ...
  _queueCheckWorkspaces() {
    if (this._checkWorkspacesId === 0) {
      ...
      this._checkWorkspacesId = laters.add(
  ...
  _nWorkspacesChanged() {
    ...
    if (oldNumWorkspaces === newNumWorkspaces) return false;
...
export const TilePreview = GObject.registerClass(
  class TilePreview extends St.Widget {
    ...
    open(window, tileRect, monitorIndex) {
      ...
      let changeMonitor =
        this._monitorIndex === -1 || this._monitorIndex !== monitorIndex;
...
export class WindowManager {
  constructor() {
    ...
    this._shellwm.connect(
      "show-tile-preview",
      this._showTilePreview.bind(this),
    );
    this._shellwm.connect(
      "hide-tile-preview",
      this._hideTilePreview.bind(this),
    );
    ...
    this._shellwm.connect(
      "filter-keybinding",
      this._filterKeybinding.bind(this),
    );
    this._shellwm.connect(
    ...
    this._shellwm.connect(
      "create-close-dialog",
      this._createCloseDialog.bind(this),
    );
    this._shellwm.connect(
      "create-inhibit-shortcuts-dialog",
    ...
    this.setCustomKeybindingHandler(
    ...
    global.display.connect(
      "show-resize-popup",
      this._showResizePopup.bind(this),
    );
    global.display.connect("show-pad-osd", this._showPadOsd.bind(this));
    global.display.connect(
      "show-osd",
      (display, monitorIndex, iconName, label) => {
        let icon = Gio.Icon.new_for_string(iconName);
        Main.osdWindowManager.show(monitorIndex, icon, label, null);
      },
    );

    this._gsdWacomProxy = new GsdWacomProxy(
      Gio.DBus.session,
      GSD_WACOM_BUS_NAME,
    ...
    global.display.connect("pad-mode-switch", (display, pad, _group, _mode) => {
      ...
      for (let i = 0; i < 50; i++) {
        let str = display.get_pad_action_label(
          pad,
      ...
      this._gsdWacomProxy
        ?.SetOLEDLabelsAsync(pad.get_device_node(), labels)
        .catch(logError);
    });
    ...
    topDragAction.connect("activated", () => {
      ...
      if (currentWindow) currentWindow.unmake_fullscreen();
    });
  ...
  _showPadOsd(display, device, settings, imagePath, editionMode, monitorIndex) {
    this._currentPadOsd = new PadOsd.PadOsd(
      device,
      settings,
      imagePath,
      editionMode,
  ...
  _lookupIndex(windows, metaWindow) {
    for (let i = 0; i < windows.length; i++) {
      if (windows[i].metaWindow === metaWindow) return i;
    }
    return -1;
  }
  ...
  _switchApp() {
    let windows = global.get_window_actors().filter((actor) => {
      ...
      return (
        !win.is_override_redirect() && win.located_on_workspace(activeWorkspace)
      );
    });
  ...
  insertWorkspace(pos) {
    ...
    if (!Meta.prefs_get_dynamic_workspaces()) return;
    ...
    windows.forEach((window) => {
      ...
      // unstick them
      if (window.on_all_workspaces) return;
  ...
  keepWorkspaceAlive(workspace, duration) {
    if (!this._workspaceTracker) return;
  ...
  _unminimizeWindow(shellwm, actor) {
    ...
    if (actor.meta_window.is_monitor_sized()) {
      ...
      if (success) {
        ...
        actor.set_scale(geom.width / actor.width, geom.height / actor.height);
      } else {
        let monitor =
          Main.layoutManager.monitors[actor.meta_window.get_monitor()];
        if (!monitor) {
  ...
  _waitForOverviewToHide() {
    if (!Main.overview.visible) return Promise.resolve();
    ...
    actor.meta_window.connectObject(
      ...
      () => {
        ...
        if (
          ...
          if (parent) this._checkDimming(parent);
        }
  ...
  _filterKeybinding(shellwm, binding) {
    if (Main.actionMode === Shell.ActionMode.NONE) return true;

    // There's little sense in implementing a keybinding in mutter and
    // not having it work in NORMAL mode; handle this case generically
    // so we don't have to explicitly allow all builtin keybindings in
    // NORMAL mode.
    if (Main.actionMode === Shell.ActionMode.NORMAL && binding.is_builtin())
      return false;
  ...
  _getNthFavoriteApp(n) {
    if (!this._allowFavoriteShortcuts()) return null;
  ...
  _showWorkspaceSwitcher(display, window, event, binding) {
    ...
    if (target === "last") {
      if (vertical) direction = Meta.MotionDirection.DOWN;
      else if (rtl) direction = Meta.MotionDirection.LEFT;
      else direction = Meta.MotionDirection.RIGHT;
      ...
      let prependTarget;
      if (vertical) prependTarget = "up";
      else if (rtl) prependTarget = "right";
      else prependTarget = "left";
      if (
        workspaceManager.get_active_workspace_index() === 0 &&
        action === "move" &&
        target === prependTarget &&
        this._isWorkspacePrepended === false
      ) {
      ...
      if (workspaceManager.get_active_workspace_index() > target) {
        if (vertical) direction = Meta.MotionDirection.UP;
        else if (rtl) direction = Meta.MotionDirection.RIGHT;
        else direction = Meta.MotionDirection.LEFT;
      } else {
        if (vertical)
    ...
    if (
      workspaceManager.layout_columns === -1 &&
      direction !== Meta.MotionDirection.LEFT &&
      direction !== Meta.MotionDirection.RIGHT
    )
      return;

    if (action === "switch") this.actionMoveWorkspace(newWs);
    else this.actionMoveWindow(window, newWs);

    if (!Main.overview.visible) {
      if (this._workspaceSwitcherPopup == null) {
        this._workspaceTracker.blockUpdates();
        this._workspaceSwitcherPopup =
  ...
  actionMoveWorkspace(workspace) {
    ...
    if (!workspace.active) workspace.activate(global.get_current_time());
  }
  ...
  handleWorkspaceScroll(event) {
    ...
    switch (direction) {
      case Clutter.ScrollDirection.UP:
        if (vertical) ws = activeWs.get_neighbor(Meta.MotionDirection.UP);
        else if (rtl) ws = activeWs.get_neighbor(Meta.MotionDirection.RIGHT);
        else ws = activeWs.get_neighbor(Meta.MotionDirection.LEFT);
        break;
      case Clutter.ScrollDirection.DOWN:
        if (vertical) ws = activeWs.get_neighbor(Meta.MotionDirection.DOWN);
        else if (rtl) ws = activeWs.get_neighbor(Meta.MotionDirection.LEFT);
        else ws = activeWs.get_neighbor(Meta.MotionDirection.RIGHT);
        break;
    ...
    GLib.timeout_add(GLib.PRIORITY_DEFAULT, SCROLL_TIMEOUT_TIME, () => {
  ...
  _showResizePopup(display, show, rect, displayW, displayH) {
    if (show) {
      if (!this._resizePopup) this._resizePopup = new ResizePopup();

      this._resizePopup.set(rect, displayW, displayH);
    } else {
      if (!this._resizePopup) return;
