...
export class WindowMenu extends PopupMenu.PopupMenu {
  ...
  _buildMenu(window) {
    ...
    if (!window.can_minimize()) item.setSensitive(false);
    ...
    if (!window.can_maximize()) item.setSensitive(false);
    ...
    if (!window.allows_move()) item.setSensitive(false);
    ...
    if (!window.allows_resize()) item.setSensitive(false);

    item = this.addAction(_("Always on Top"), () => {
      if (window.is_above()) window.unmake_above();
      else window.make_above();
    });
    if (window.is_above()) item.setOrnament(PopupMenu.Ornament.CHECK);
    if (
    ...
    if (
      Main.sessionMode.hasWorkspaces &&
      ...
      item = this.addAction(_("Always on Visible Workspace"), () => {
        if (isSticky) window.unstick();
        else window.stick();
      });
      if (isSticky) item.setOrnament(PopupMenu.Ornament.CHECK);
      if (window.is_always_on_all_workspaces()) item.setSensitive(false);
    ...
    if (nMonitors > 1 && monitorIndex >= 0) {
      ...
      let upMonitorIndex = display.get_monitor_neighbor_index(
        monitorIndex,
        dir,
      );
      if (upMonitorIndex !== -1) {
      ...
      let downMonitorIndex = display.get_monitor_neighbor_index(
        monitorIndex,
        dir,
      );
      ...
      let leftMonitorIndex = display.get_monitor_neighbor_index(
        monitorIndex,
        dir,
      );
      ...
      let rightMonitorIndex = display.get_monitor_neighbor_index(
        monitorIndex,
        dir,
      );
    ...
    if (!window.can_close()) item.setSensitive(false);
  }
...
export class WindowMenuManager {
  constructor() {
    this._manager = new PopupMenu.PopupMenuManager(
  ...
  showWindowMenuForWindow(window, type, rect) {
    if (!Main.sessionMode.hasWmMenus) return;
