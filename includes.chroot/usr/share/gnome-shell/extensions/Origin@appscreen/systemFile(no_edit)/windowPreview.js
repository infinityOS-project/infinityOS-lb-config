...
export const WindowPreview = GObject.registerClass(
  ...
  class WindowPreview extends Shell.WindowPreview {
    _init(metaWindow, workspace, overviewAdjustment) {
      ...
      windowContainer.layout_manager.connect(
        ...
        (layout) => {
          ...
          if (layout.bounding_box.get_area() > 0) this.emit("size-changed");
        },
      );
      ...
      clickAction.connect("long-press", (action, actor, state) => {
        if (state === Clutter.LongPressState.ACTIVATE) this.showOverlay(true);
        return true;
      });
      ...
      this._closeButtonSide = layout.left_buttons.includes(
    ...
    _updateIconScale() {
      ...
      const scale = visible
        ? 1 - Math.abs(ControlsState.WINDOW_PICKER - currentState)
        : 0;
    ...
    _windowCanClose() {
      return this.metaWindow.can_close() && !this._hasAttachedDialogs();
    }
    ...
    chromeHeights() {
      ...
      return [topOversize + activeExtraSize, bottomOversize + activeExtraSize];
    }
    ...
    chromeWidths() {
      ...
      const leftOversize =
        this._closeButtonSide === St.Side.LEFT ? closeButtonWidth / 2 : 0;
      const rightOversize =
        this._closeButtonSide === St.Side.LEFT ? 0 : closeButtonWidth / 2;

      return [leftOversize + activeExtraSize, rightOversize + activeExtraSize];
    }
    ...
    addDialog(win) {
      ...
      while (parent.is_attached_dialog()) parent = parent.get_transient_for();
      ...
      if (this._closeRequested) this._activate();
    }
    ...
    vfunc_leave_event(event) {
      ...
      GLib.Source.set_name_by_id(
        this._idleHideOverlayId,
        "[gnome-shell] this._idleHideOverlayId",
      );
    ...
    _restack() {
      ...
      if (parent !== null) {
        if (this._overlayShown) {
          ...
          if (above === null) parent.set_child_below_sibling(this, null);
          else parent.set_child_above_sibling(this, above);
        }
      }
    }
    ...
    _onDragEnd(_draggable, _time, _snapback) {
      ...
      if (this["has-pointer"]) this.showOverlay(true);
